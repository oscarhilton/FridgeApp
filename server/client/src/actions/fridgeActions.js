...
import { CREATE_FRIDGE, GET_MY_FRIDGES, SET_USER_LIST, GET_FRIDGE_BY_ID } from './types';
...
export const getFridgeById = (id) => async dispatch => {
  const res = await axios.post('/api/fridge/findById', {id} );
  console.log(res);
  dispatch({ type: GET_FRIDGE_BY_ID, payload: res.data })
}
